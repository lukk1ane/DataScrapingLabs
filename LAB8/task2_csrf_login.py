#!/usr/bin/env python3
"""
LAB8 Task 2: CSRF Token Login Form Scraping
Handle login form with CSRF token protection.
"""
...
def extract_csrf_token(session, login_url):
    ...
    try:
        ...
        # Common CSRF token field names and selectors
        csrf_selectors = [
            'input[name="_token"]',
        ...
        for selector in csrf_selectors:
            ...
            if csrf_element:
                ...
                if token:
                    ...
                    print(f"✅ Found CSRF token: {token[:20]}... (field: {field_name})")
...
def get_demo_credentials(session, login_url):
    ...
    try:
        ...
        if demo_info:
            ...
            print(f"Demo credentials info: {text}")
        
        # Default credentials for scraping courses
        return {
...
def extract_form_data(session, login_url):
    """Extract form action and field names from the login form."""
    try:
        response = session.get(login_url)
        response.raise_for_status()
...
def login_with_csrf_and_get_products(login_url, products_url):
    """Perform login with CSRF token and extract products."""
    ...
    try:
        ...
        print("\nStep 4: Submitting login form with CSRF token...")
        login_data = {
...
def main():
    ...
    print("🚀 LAB8 Task 2: CSRF Token Login Form Scraping")
    print("=" * 50)
    
    login_url = "https://www.scrapingcourse.com/login/csrf"
    products_url = "https://www.scrapingcourse.com/ecommerce"
    
    success = login_with_csrf_and_get_products(login_url, products_url)
